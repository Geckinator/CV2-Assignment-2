function [fundamental_matrix] = compute_fundamental_matrix(image1, image2, is_ransac, threshold, epochs)
...
if is_ransac
    ...
    A(:, 1) = coordinates1(1, :).*coordinates2(1, :);
    A(:, 2) = coordinates1(1, :).*coordinates2(2, :);
    A(:, 3) = coordinates1(1, :);
    A(:, 4) = coordinates1(2, :).*coordinates2(1, :);
    A(:, 5) = coordinates1(2, :).*coordinates2(2, :);
    A(:, 6) = coordinates1(2, :);
    A(:, 7) = coordinates2(1, :);
    A(:, 8) = coordinates2(2, :);
    ...
    % Find F from the column of V that corresponds to the smallest singular
    % value:
    ...
    S(3, 3) = 0;
    fundamental_matrix = T2'*U*S*V'*T1;
end
